...
def _system_prompt(repo: Path) -> str:
    ...
    return (
        ...
        "Work to a plan:\n"
        "- Start by calling plan_add once with the sub-goals you intend to work "
        "through, in order. The plan is durable: it survives context compaction, "
        "so on a long task you will still have it after early details are elided.\n"
        "- Call plan_resolve the moment a sub-goal is genuinely done. Never re-open "
        "or redo a resolved sub-goal — if you are unsure whether something is "
        "finished, consult the plan rather than repeating the work.\n"
        "- Keep working the plan through tool calls. While sub-goals remain "
        "unresolved, every turn must call a tool — never reply with prose, a "
        "summary, or a description of what you are about to do. Narrating is not "
        "acting: say it with a tool call or call finish.\n"
        "Work safely:\n"
